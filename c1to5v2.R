...
getBolds <- function(){
  b_container <- character()
  list(b = function(node, ...){
    b_container <<- c(b_container, xmlValue(node))
  }, returnB = function() b_container
  )
}

h4 <- getBolds()

htmlTreeParse(url, handlers = h4)
...
info <- debugGatherer()
#info <- getCurlHandle()
problemsD <- getCurlHandle(autoreferer = TRUE, cookiefile = ""
                           , httpheader= 
                             list(from = "franckx.laurent@gmail.com", followlocation = TRUE, 
                                  'user-agent'= str_c(R.version$platform,  R.version$version.string,sep=",")
                                   )
                           , debugfunction = info$update, verbose = T)
...
for(i in seq_along(urls)){
  urli <- getURL(urls[i])
  urli_cont <- str_split(urli,"\n")[[1]]
  filename <- paste("geturl",i,".html",sep="")
  writeLines(urli, con = filename )
}

for(i in seq_along(urls)){
  urli <- getURLContent(urls[i])
  urli_cont <- str_split(urli,"\n")[[1]]
  filename <- paste("geturlcontent",i,".html",sep="")
  writeLines(urli_cont, con = filename
       #, ascii = TRUE
  )
}
...
for(i in seq_along(urls)){
  urli <- getBinaryURL(urls[i])
  filename <- paste("getbinaryurl",i,".html",sep="")
  save(urli, file = filename
       #, ascii = TRUE
  )
}

for(i in seq_along(urls)){
  filename <- paste("downloadfile",i,".html",sep="")
  urli <- download.file(urls[i], filename)
}
...
cat(getForm("http://www.r-datacollection.com/materials/http/return.php", name = "Laurent", submitButton = "Now!"
        , textarea = "some text to send"))


cat(postForm("http://www.r-datacollection.com/materials/http/return.php", name = "Laurent", submitButton = "Now!"
            , textarea = "some text to send"))
