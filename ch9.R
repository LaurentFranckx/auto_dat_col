...
cart  <- htmlParse(getURL(url=cart_url, curl=handle)) 
clean <- function(x)  str_replace_all(xmlValue(x),"(\t)|(\n)"," ") 
xpathSApply(cart, "//h3/a")
xpathSApply(cart, "//h3/a", clean)
#uncleaned version 
xmlValue(xpathSApply(cart, "//h3/a")[[1]])
...
# dj0yJmk9TmNzVVhQbjI4UXhNJmQ9WVdrOWRWRkVSbTFZTlRnbWNHbzlNQS0tJnM9Y29uc3VtZXJzZWNyZXQmeD0yYw--
#   Client Secret (Consumer Secret)
# 45a40466ecc526b1f9cb2f170d858417ee32df3d

options(yahooid = "dj0yJmk9TmNzVVhQbjI4UXhNJmQ9WVdrOWRWRkVSbTFZTlRnbWNHbzlNQS0tJnM9Y29uc3VtZXJzZWNyZXQmeD0yYw")
...
berchem_url <- sprintf(baseurl, URLencode("Berchem, Belgium"))
parsed_berchem <- xmlParse((getForm(berchem_url, appid = getOption("yahooid"))))
xpathSApply(parsed_berchem, "//*[local-name()='locality1']", xmlAttrs)
...
Sys.setenv(FACEBOOK_CONSUMER_SECRET = "fe4b1135b54479d34e6d5d51e9b4ff51") # add consumer secret here

# bundly consumer key and secret
fb_app <- oauth_app("facebook", "795037247232494")
...
# friends_relationships, friends_status, friends_checkins, 
publish_actions,
read_stream
#,export_stream
"
# set permissions
permissions <- "user_birthday, user_hometown, user_location, user_status,publish_actions,read_stream"
